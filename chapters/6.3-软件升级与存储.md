...
**A/B分区**是优雅的解法。Flash被分成两个大小相等的独立分区——A区和B区。当前固件运行在A区。OTA下载的新固件全部写入B区。A区在整个下载和验证过程中从未被触碰，它完好地保存着你开始升级之前那一刻的完整系统状态。

下载完成后，系统验证B区固件的签名和完整性。验证通过后，在一个非易失标志中写“下次启动请选择B区”。然后复位。
...
A/B 分区的控制核心是一个微小的**启动引导器（Bootloader）**，它在这个固件升级的宇宙里是唯一“不参与轮换”的代码。它被放在 Flash 的某个独立区域，不随 A/B 切换而改变。
...
Bootloader 的代码量非常小，通常小于 4KB。它不参与 A/B 轮换，始终保持在 Flash 的某个独立区域里。它的逻辑极其简单：读标志、选分区、验签名、跳转。简单的逻辑意味着更少的 bug，而 bootloader 里的任何一个 bug 都可能让 ECU 永远无法启动。
...
OTA 状态机的代码量比 bootloader 大得多，但它仍然遵循一个清晰的原则：**在不可靠的网络上可靠地传输数据。** 每个 chunk 独立做 SHA-256 校验，确保一个 chunk 校验失败不会影响之前已确认的 chunk。掉电后可以从上次保存的进度恢复，不需要从头开始。SHA-256 和签名验证都交给 HSM，主 CPU 不做密码运算。
...
磨损均衡不是 OTA 的事，OTA 是整块分区擦除重写。磨损均衡是 NvM（非易失存储管理）的事：DTC 记录、标定数据、学习值，这些频繁写入的小数据块需要磨损均衡保护 Flash 寿命。
...
一辆车出厂后，在接下来的 15 年里，它不会再回到你的工厂。但它可能会跑 50 万公里。经过赤道和北极圈。被颠簸得螺丝松动。被电磁干扰轰击。被雷击中的物体的电流脉冲感应。被泡过水。被换了三手车主。被在路边摊改过电路。被停在地下车库三年没动。

在这 15 年里，你可能只有一次机会给它升级——通过 OTA。没有物理接触，没有 JTAG 调试器，没有产线编程器。

你必须信任你的 A/B 分区，B 区坏了回 A 区。你必须信任你的 HSM 签名验证，被篡改的固件不会启动。你必须信任你的 Flash 掉电保护，断电也不会丢数据。你必须信任你的回滚计数器，旧漏洞不会重生。

一个 ECU，它在不可靠的电源、不可靠的网络、不可预知的物理环境中运行。但你必须保证：在所有这些不确定性中，**变砖的概率无限接近零。**
...
3. **这是不可逆的承诺**：一辆车在出厂后15年里不会再回工厂。你可能只有一次机会通过OTA给它升级——你赌的是，当年写的那段升级逻辑不会在任何一天害死这辆车。
4. **NvM 块管理、磨损均衡算法、Flash EEPROM 仿真的深层实现细节**：见姊妹篇[《存储技术书》](https://github.com/Lularible/storage-book)。
